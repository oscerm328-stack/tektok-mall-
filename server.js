...
/* CART BADGE */
.cart-badge-wrap{position:relative;display:inline-flex;align-items:center;cursor:pointer;}
...
.cart-item-price{font-size:13px;color:#e53935;font-weight:700;}
...
.cart-total-val{font-size:17px;font-weight:800;color:#e53935;}
...
<div class="header">
  ...
  <div class="h-right">
    ...
    <span class="cart-badge-wrap h-icon" onclick="openCartDrawer()">
      <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
...
<div class="bottom-bar">
  ...
  <div class="bar-icon" onclick="openCartDrawer()">
...
async function doCart(){
    if(!p) return;
    var cart = JSON.parse(localStorage.getItem("__cart")||"[]");
    var exists = cart.find(function(i){ return i.id === p.id; });
    if(exists){ closeSheet(); showToast("⚠️ Already in cart"); return; }
...
function saveCart(c){ localStorage.setItem("__cart", JSON.stringify(c)); updateCartBadge(); }
...
function renderCartDrawer(){
    ...
    cart.forEach(function(item, idx){
        var catF = ({17:"17_Clothing_and_Accessories",19:"19_Medical_Bags_and_Sunglasses",20:"20_Shoes",21:"21_Watches",22:"22_Jewelry",27:"27_Electronics",28:"28_Smart_Home",31:"31_Luxury_Brands",32:"32_Beauty_and_Personal_Care",34:"34_Mens_Fashion",35:"35_Health_and_Household",36:"36_Home_and_Kitchen"})[item.category_id]||"27_Electronics";
        var imgSrc = "https://res.cloudinary.com/doabtbdsh/image/upload/products/"+catF+"/"+(item.folder||"")+"/1.jpg";
        ...
        row.innerHTML =
            ...
            '<div class="cart-item-info">' +
              '<div class="cart-item-name">'+(item.title||"").replace(/&/g,"&amp;").replace(/</g,"&lt;")+'</div>' +
              '<div class="cart-item-price">US$'+parseFloat(item.price).toFixed(2)+'</div>' +
            '</div>' +
            '<div class="cart-item-del" onclick="removeCartItem('+idx+')">✕</div>';
...
function removeCartItem(idx){
    var cart = getCart(); cart.splice(idx,1); saveCart(cart); renderCartDrawer();
}
...
async function cartCheckout(){
    ...
    try {
        ...
        var r = await fetch("/create-store-order", {
            method:"POST",
            headers:{"Content-Type":"application/json","Authorization":"Bearer "+token},
            body: JSON.stringify({ products: selected, sellerEmail: sEmail })
        });
        var d = await r.json();
        if(d.success){
